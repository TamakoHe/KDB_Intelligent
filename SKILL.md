...
```js
// read.batteries 返回基础表当前快照；faultStatus 不是历史事件查询。
...
历史故障筛选：`read.history` 每次必须指定一个明确的 8 位电池 ID。可以先用基础表取得候选电池快照，再逐个读取历史、按故障码和时间范围筛选，最后导出命中的电池；这不是对所有历史分表的隐式全库扫描，结果必须说明候选清单范围和可能遗漏。不能把当前基础表的 `faultStatus=108` 描述成“最近两周出现过充电高温”。分析脚本应让桌面端显示逐电池扫描进度，并支持用户停止任务。
...
### Resumable full-library scans and expert SQL

For requests that explicitly need all historical partitions, cross-generation scans, or a task that may run longer than the normal analysis limits, use `run_kdb_scan`. Submit only `purpose`, `start`, `end`, `generations`, `source` (`api`, `local`, or `both`), and whitelist filters. The desktop host enumerates only the known Gen2/Gen3 history schemas and table prefixes, processes 100 targets per batch with at most four concurrent reads, persists a cursor in the app database, and supports pause/resume/stop after restart. API and local results remain separately labelled; skipped tables and transient failures are reported instead of being hidden. The scan has no fixed total row limit, but each batch is bounded and writes detail files incrementally.

For a user explicitly requesting a complex local-only query, use `run_kdb_sql` to create a preview. It accepts one `SELECT` or `WITH ... SELECT` statement only. Never generate writes, multiple statements, comments, file output, unknown schemas, control commands, OTA commands, or shell/Node code. The desktop host validates and evaluates the query before showing the execution button, then writes results to an Excel file. The configured database account is used; remind users that a dedicated read-only account is safer than root.
